...
class TitulkyFreeSubtitle(Subtitle):
...
class TitulkyFreeProvider(Provider, ProviderSubtitleArchiveMixin):
    ...
    subtitle_class = TitulkyFreeSubtitle
    ...
    def initialize(self):
        ...
        # Set headers
        cached_user_agent = cache.get('titulky-free_user_agent')
        if cached_user_agent == NO_VALUE:
            new_user_agent = AGENT_LIST[randint(0, len(AGENT_LIST) - 1)]
            cache.set('titulky-free_user_agent', new_user_agent)
    ...
    def query_show(self, languages, imdb_id, season, episode):
        search_params = {
            serial2: imdb_id,
            serial: ''
        }
        
        search_response = self.session.get(...., search_params)
